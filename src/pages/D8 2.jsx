import React from 'react'
import { ConServ, lineLogo } from '../assets'
import ButtonSm from '../components/ButtonSm'

function D8() {
  return (
    <div className="flex flex-col items-center">
      <header className='flex flex-col items-center'>
        <a href="/Home"><img src={lineLogo} alt="Line Logo" /></a>
        <p className="flex justify-center">Intra in meniul monitorului apasand butonul “MENU” de pe telecomanda.</p>
        <img src={ConServ} alt="Remote" className='max-w-[320px] max-h-[500px]' />
      </header>

      <p className="my-[30px]">Daca este deja introdusa contactati-ne!<br /> <span className="spectext">+40215394507</span></p>


      <div className='flex flex-row'>
        <a href="/D7" className='flex justify-start mr-[40px]'><ButtonSm text="Inapoi"></ButtonSm></a>
        <a href="/Home" className='flex justify-end'><ButtonSm text="Finalizare"></ButtonSm></a>
      </div>
      
    </div>
  )
}

export default D8